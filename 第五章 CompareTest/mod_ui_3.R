mod_ui_3 <- function(id) {
  ...
  tagList(
    sidebarLayout(
      sidebarPanel(
        h4("输入有序或连续数据"),
        textInput(ns("data0"), "未患病个体的数据 (data0):", value = "136,286,281,23,200,146,220,96,100,60,17,27,126,100,253,70,40,6,46"),
        textInput(ns("data1"), "患病个体的数据 (data1):", value = "140,1087,230,183,1256,700,16,800,253,740,126,153,283,90,303,193,76,1370,543,913,230,463,60,509,576,671,80,490,156,356,350,323,1560,120,216,443,523,76,303,353,206"),
        numericInput(ns("alpha"), "显著性水平 (α):", value = 0.05, min = 0, max = 1),
        actionButton(ns("submit"), "提交")
      ),
      mainPanel(
        h4("分析结果"),
        h4("params_estimate_ordinal_unpaired_result"),
        verbatimTextOutput(ns("params_estimate_ordinal_unpaired_result")),
        h4("BoxCox_result"),
        verbatimTextOutput(ns("BoxCox_result")),
        h4("params_estimate_continuous_unpaired_result"),
        verbatimTextOutput(ns("params_estimate_continuous_unpaired_result"))
      )
    )
  )
}
