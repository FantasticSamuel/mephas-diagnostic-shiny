...
#谢飞彪同学提供的随机森林的roc绘制函数
# roc_plot0022222222 <- function(lab, prob) {
#   
#   rocobj <- roc(lab, prob, smooth = F)
#   
#   cutOffPoint <- coords(rocobj, "best")
#   cutOffPointText <- paste0(
#     round(cutOffPoint[1], 3),
#     "(",
#     round(cutOffPoint[2], 3),
#     ",",
#     round(cutOffPoint[3], 3),
#     ")"
#   )
#   
#   auc <- auc(rocobj)[1]
...
#       # 当legacy.axes=TRUE时， 把x=x改为x=1-x
#       fill = 'lightblue',
#       alpha = 0.5
#     ) +
#     geom_point(aes(x = cutOffPoint[[2]], y = cutOffPoint[[3]])) + # 绘制临界点/阈值
#     geom_text(aes(x = cutOffPoint[[2]], y = cutOffPoint[[3]], label = cutOffPointText),
#               vjust = -1) + # 添加临界点/阈值文字标签
#     annotate(
#       "text",
#       x = 0.6,
#       y = 0.6,
#       label = paste("AUC = ", round(auc, 4), sep = ""),
#       size = 3,
#       fontface = "bold",
#       family = "A"
#     )
...
#roc绘制函数
roc_plot3 <- function(lab, prob, thr) {
  ...
  tss <- coords(rocobj) %>%
    .[!is.infinite(.$threshold),]
  
  cutOffPoint <- tss[which.min(abs(tss$threshold - thr)),]
  # cutOffPointText <- paste0(
  #   round(thr, 3),
  #   "(",
  #   round(cutOffPoint[1,2], 3),
  #   ",",
  #   round(cutOffPoint[1,3], 3),
  #   ")"
  # )
  ...
  auc_val <- auc(lab, prob,)
  ci <- ci(auc(lab, prob,), conf.level = 0.95)
